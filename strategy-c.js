...
// 2거래소 가격 비율 sanity — 한쪽이 다른쪽의 70%~143% 범위 벗어나면 단위 오류/상폐/리스팅 직후
// (이전 0.5~2.0은 너무 헐거움 — DRIFT 75.5→110 같은 비정상 +45% spread 통과)
const MIN_PRICE_RATIO = 0.70;
const MAX_PRICE_RATIO = 1.43;
// 가격 절대 플로어 — KRW 1원 미만 코인은 호가갭으로 spread% 왜곡 (BTT 0→0.001 = +∞%)
const MIN_PRICE_KRW = 1.0;
// 비정상적으로 큰 호가갭(bid/ask 비율) 차단 — 호가창 거의 비어있어 체결 불가
const MAX_INTRA_EX_GAP = 0.05; // 동일 거래소 호가갭 5% 초과 → reject (필요시 호가 데이터 있을 때만 적용)
...
function netProfit(priceBuy, priceSell, exA, exB) {
  // 1. 양쪽 가격 0/음수 → 즉시 reject
  if (!Number.isFinite(priceBuy) || !Number.isFinite(priceSell)) return -1;
  if (priceBuy <= 0 || priceSell <= 0) return -1;

  // 2. 가격 절대 플로어 — KRW 1원 미만은 호가단위(0.001) 때문에 spread% 왜곡
  //    BTT 0.0008→0.001 같은 케이스가 +25% 차익으로 표시되지만 사실상 체결 불가능
  if (priceBuy < MIN_PRICE_KRW || priceSell < MIN_PRICE_KRW) return -1;

  // 3. 가격 비율 sanity (단위 오류/상폐/리스팅 직후 차단)
  ...
  // 4. 비정상적으로 큰 차익 차단 (5% 초과 = 호가창 거의 빈 상태/데이터 오류)
